...
# if there are certain number of symptoms common between two patients
...
df =  data[,c(1:27)]
df
#  -----------  matching different rows ---------------
check_matching <- function(left, rigth, no_of_symptoms){
  matching_symptoms_count <- length((which(left == rigth) == TRUE))
  print(matching_symptoms_count)
  if (matching_symptoms_count >= no_of_symptoms){
    print('matched')
    return(1)
  }
  else{
